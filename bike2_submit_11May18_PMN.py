# -*- coding: utf-8 -*-
"""
Created on Fri May 11 09:25:44 2018
...
def load_data(city, month, day):
    ...
    df = pd.read_csv(CITY_DATA[city],parse_dates=True,index_col='Start Time') #code from Datacamp course on Pandas
    #use time indexing to generate the trip duration graphs of daily means
    df['Start Time'] = df.index #add start time column back in
    df_copy = df
...
def time_stats(df):
    ...
    plt.xlabel('Hour (24 hour clock)') #not sure why some months (May,June) in Washington only first 12 hours.
    plt.show()
    print('-'*40)
